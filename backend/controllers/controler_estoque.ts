...
export default class Controller_Estoque {
    ...
    static async BloquearSaldo(req: Request, res: Response) {
        ...
        try {
            ...
            const est_id = Number(req.params.est_id) || 0;
            const est_bloquear = Number(req.body.est_bloquear) || 0;

            if (est_id === 0) {
                const error = new Error('ID do estoque não informado.') as any;
                error.statusCode = 400;
                throw error;
            }

            if (est_bloquear <= 0) {
                const error = new Error('Quantidade para bloqueio deve ser maior que zero.') as any;
                error.statusCode = 400;
                throw error;
            }
            ...
            if (!estoque.found) {
                const error = new Error('ID de Estoque não encontrado.') as any;
                error.statusCode = 404;
                throw error;
            }

            if (estoque.est_saldo_disponivel < est_bloquear) {
                const error = new Error(`Saldo insuficiente para bloqueio.`) as any;
                error.statusCode = 400;
                throw error;
            }

            estoque.est_saldo_disponivel -= est_bloquear;
            estoque.est_saldo_bloqueado += est_bloquear;

            await estoque.Salvar();

            resdata.msg = "Saldo bloqueado com sucesso";
    ...
    static async DesBloquearSaldo(req: Request, res: Response) {
        ...
        try {
            ...
            const est_id = Number(req.params.est_id) || 0;
            const est_desbloquear = Number(req.body.est_desbloquear) || 0;
            ...
            if (est_desbloquear <= 0) {
                const error = new Error('Quantidade para desbloqueio deve ser maior que zero.') as any;
                error.statusCode = 400;
                throw error;
            }
            ...
            if (estoque.est_saldo_bloqueado < est_desbloquear) {
                const error = new Error(`Saldo insuficiente para desbloqueio.`) as any;
                error.statusCode = 400;
                throw error;
            }

            estoque.est_saldo_disponivel += est_desbloquear;
            estoque.est_saldo_bloqueado -= est_desbloquear;

            await estoque.Salvar();

            resdata.msg = "Saldo desbloqueado com sucesso";
